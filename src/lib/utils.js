...
export function formatUptime(sec) {
    const { days, hours, minutes, seconds } = msToTime(sec * 1000);
    return [days && `${days}h`, hours && `${hours}j`, minutes && `${minutes}m`, `${seconds}d`].filter(Boolean).join(' ');
}

export function formatUptimeFull(ms) {
    const { days, hours, minutes, seconds } = msToTime(ms);
...
export async function fetchJson(url, options = {}) {
    const { headers = {}, timeout = 30000, method = 'GET', body } = options;
...
export async function fetchBuffer(url, options = {}) {
    ...
    if (!res.ok) throw new Error(`fetchBuffer: HTTP ${res.status} ${url}`);
