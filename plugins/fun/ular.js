...
export default plugin('ular', 'snake')
    ...
    .run(async (sock, { raw, from, pushname, db, primaryId }) => {
        ...
        try {
            await sendInlineWebUI(sock, from, html, '🐍 Snake');
        } catch (e) {
...
<h1>🐍 Snake</h1>
<div class="meta"><b id="nm"></b> · Score <b id="sc">0</b> · <span id="ln">2</span></div>
<div id="board"></div>
<div class="dpad">
...
<div class="bar"><button id="go" type="button">▶ Start / Restart</button></div>
<div id="msg">Press ▶ then steer</div>
<script>
(function(){
...
function reset(){clearInterval(timer);timer=null;var m=Math.floor(S/2);snake=[{x:m,y:m},{x:m-1,y:m}];dir='R';nd='R';food=rf();score=0;dead=false;run=false;sc.textContent='0';ln.textContent=snake.length;dirEl.textContent=ic[dir];msg.className='';msg.textContent='Press ▶ then steer';draw()}
...
function die(){dead=true;run=false;clearInterval(timer);timer=null;msg.className='dead';msg.textContent='💀 Hit itself · '+score}
function win(){dead=true;run=false;clearInterval(timer);timer=null;msg.className='win';msg.textContent='🏆 You won! · '+score}
function setD(d){var o={U:'D',D:'U',L:'R',R:'L'};if(d===o[dir]&&snake.length>1)return;nd=d;if(!run&&!dead){run=true;msg.className='';msg.textContent='Playing...';timer=setInterval(step,T)}}
